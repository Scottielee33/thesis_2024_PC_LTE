...
def adjusted_perform_anova(df, quantiles, component_name):
    results = []
    for quantile in quantiles:
        # Filter data for the current quantile
        quantile_data = df[df['Quantile'] == quantile]
        # Group data by release date and extract release prices
        grouped_data = quantile_data.groupby('ReleaseDate')['AdjustedReleasePrice'].apply(list)
        # Perform ANOVA
        f_val, p_val = stats.f_oneway(*grouped_data)
        results.append({'Component': component_name, 'Quantile': quantile, 'F-Value': f_val, 'P-Value': p_val})
    return results
...
# Perform ANOVA test for each component
cpu_anova_results = adjusted_perform_anova(cpu_df, cpu_quantiles, 'CPU')
disk_anova_results = adjusted_perform_anova(disk_df, disk_quantiles, 'Disk')
gpu_anova_results = adjusted_perform_anova(gpu_df, gpu_quantiles, 'GPU')
memory_anova_results = adjusted_perform_anova(memory_df, memory_quantiles, 'Memory')

# Combine results into one dataframe
anova_results = pd.DataFrame(cpu_anova_results + disk_anova_results + gpu_anova_results + memory_anova_results)

# Save the results to a CSV file
anova_results.to_csv('adjusted_anova_results.csv', index=False)
